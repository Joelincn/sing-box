...
注意：空条目启动时直接报错（单个 `[{}]` 会在校验前被配置解析器静默丢弃，这是上游对所有列表配置的通用行为——每个条目务必写至少一个条件）；豁免只保"不断开"，被剔节点照样不接新连接。域名条件要求域名可见（经 DNS、Fake-IP 反查或嗅探）；无可见域名的连接永远不命中域名条件。`package_name` 仅 Android 有效，`process_name`/`process_path` 仅桌面有效。
