...
## 五、reF1nd-testing（1.15）基线升级 port（2026-09-08）

- 新基线：`upstream/reF1nd-testing@49228ae9c`（即 v1.15.0-alpha.2-reF1nd 构建源）。
  注意：不要跟 `testing`/`stable`（SagerNet 1.15 新架构线，已删 loadbalance
  与 DNS 组）；也不要跟 `reF1nd-stable`（1.14 旧线）。
- local `testing` 已 reset 到 `reF1nd-testing` 并 force-push。
- `mustang-1.14.0` 保持钉在旧基线（1.14 fallback），本次不动。

### port 清单（staging: rebuild/mustang-rt → mustang）

- DNS round_robin 组策略：直搬（`dns/transport/group.go`、`option/dns.go`、
  中英文檔，新旧基线三处选项文件内容一致，`git apply` 零冲突）。
- LB `exclude_threshold` 熔断 + `interrupt_exist_connections` 掐断 +
  `interrupt_exclude` 六条件豁免：port 到新 LB 结构。新基线 LB 拨测已改用
  共享 `URLTestOutbounds` + 组均值 history；熔断计数仍走业务拨号失败路径
  （`recordFailure`，跳过 `context.Canceled`），15 分钟窗口；策略 gating 由
  `AliveForTestUrl` 改为 `isAvailable`（存活且未剔除）；`RealTag` 换新签名。
- 嵌套就绪补丁（urltest skip/reuse/retry + LB 两刀 + 两个 *_nested_test.go）：
  **退役**。上游 `66beaf541`（本线已含）递归实测替代了全部需求：
  urltest 子组自己测、LB 子组成员全测、组均值 history，无盲选、无单样本、
  无互删。上游 `loadbalance_urltest_test.go`、`urltest_recursive_test.go` 覆盖。

### 验证

- `go vet` 干净；`protocol/group` 全绿（含新增 `loadbalance_health_test.go`
  4 个：阈值剔除/取消忽略/默认关闭/叶子判定）；`dns/...` 全绿
  （`dns/transport/quic` setup 失败为环境预置问题，干净基线同样失败）。
- 三平台构建（见 mustang 合并提交）。

## 六、alpha.4 基线升级

- 新基线：`upstream/reF1nd-testing@fd9b4375a`（含 alpha.3/alpha.4：provider
  parser 修复、SFA 连接列表空白修复、Android auto-redirect 完整版 + CPU
  满载修复）。
- 上游本次与补丁文件的交集仅 4 行：中断 API 改名
  `IsProviderConnectionFromContext` → `IsResourceDownloadFromContext`
